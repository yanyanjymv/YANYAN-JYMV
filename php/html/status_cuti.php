...
<div class="sidebar">
    ...
    <table class="data-table">
        ...
        $stmt = $conn->prepare("select * from cuti where `departement` =? and `username` = ? and `level` = ? and `status` in ('Pending Spv Approval','Pending Admin Approval')");
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['status']}</td>
                        <td>
                <form action='delete_cuti.php' method='POST' onsubmit='return confirm(\"Apakah Anda yakin ingin menghapus pengajuan cuti ini?\");'>
