...
<html>
	...
	<body>
		...
		<div id="slider">
			<div class="center">
				<h3>Klik tawarkan di pojok kanan untuk menawarkan mobil anda kepada kami

				<h2>Tawarkan Mobil anda kepada kami dengan menyertakan alamat, nomor yang bisa dihubungi pada kolom informasi<h2>
				<h2>Nanti akan kami hubungi apabila Kami Minat Untuk Meminang Mobil Anda<h2>
				<br>
		...
		<!-- CONTENT -->
		<div id="menumobil">
	<form action="tambah_data_penawaran.php" method="post">
  <input type="submit" name="submit" class="search btn-cari" value="Tawarkan"> 
</form>

	</form>
		<div class="judulmenu"> Menu Penawaran </div>	
	</div>
	...
	<?php 
        ...
        if($submit && $search != ""){
            $query = mysqli_query($konekdb, "SELECT * FROM tb_penawaran WHERE nm_mobil LIKE '%$search%' ORDER BY id_mobil ASC");
        } else {
            $query = mysqli_query($konekdb, "SELECT * FROM tb_penawaran WHERE id_user='$id_show' ORDER BY id_penawaran ASC");
        }
    ...
    ?>
		...
		<table class="tableisi">
			...
			<tr> <td class="tdspek"> <?php echo $data['informasi']; ?> </td> </tr>
			<tr> <td class="tdharga"> RP.<?php echo number_format($data['harga_mobil'],0,",",".") ;?>  </td> </tr>
			<tr> <td class="tdaksi"> <a class="btn-beli" href="editmobil.php?id_penawaran=<?php echo $data['id_penawaran'];?>"> edit </a> 
