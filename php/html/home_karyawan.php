...
<?php
    ...
    $stmtPending = $conn->prepare("SELECT COUNT(*) as Pending FROM cuti WHERE `departement` = ? AND `username` = ? AND `status` in ('Pending Spv Approval', 'Pending Admin Approval')");
    if (!$stmtPending) {
...
<body>
    ...
    <div id="current-date" style="text-align: center; margin-bottom: 20px; font-size: 14px;"></div>
       <hr>
        <ul class="menu">
            <li><a href="home_karyawan.php"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
